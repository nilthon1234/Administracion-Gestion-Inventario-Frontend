...
@Component({
  ...
  templateUrl: './stock-vitrina.component.html',
  styleUrl: './stock-vitrina.component.css'
})
export class StockVitrinaComponent {
  private vitrinaService = inject(VitrinaAService);
  ...
  loadData() {
    ...
    this.vitrinaService.getOutOfStock(this.pageNumber(), this.pageSize()).subscribe(response => {
  ...
  generatePdf() {
    ...
    const title = 'Listado de Productos Sin Stock VITRINA';
    ...
    doc.save(`Producto Vitrina -sin-stock-pagina-${currentPage}.pdf`);
  }
