...
@Component({
  selector: 'app-observaciones-vitrina',
  standalone: true,
  imports: [NavbarsVisualizadorVitrinaComponent,CopiarTextoDirective, CommonModule, FormsModule],
  templateUrl: './observaciones-vitrina.component.html',
  styleUrl: './observaciones-vitrina.component.css'
})
export class ObservacionesVitrinaComponent {
  ...
  constructor(private vitrinaService: VitrinaAService) {
    this.loadData();
  }

  loadData(): void {
    this.vitrinaResponse$ = this.vitrinaService.getVitrinaFaltantes(this.currentPage, this.pageSize);
  ...
  async generatePDF(): Promise<void> {
    try {
      ...
      pdf.setFontSize(16);
      pdf.text('Productos Por Mostrar En Vitrina', 14, 10);
