...
@Component({
  selector: 'app-filter-vitrina-slipper',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, FormsModule, SizeComponent,
    MatPaginatorModule, CopiarTextoDirective, NavbarsVitrinasBComponent, SizeFormatPipe],
  templateUrl: './filter-vitrina-slipper.component.html',
  styleUrl: './filter-vitrina-slipper.component.css'
})
export class FilterVitrinaSlipperComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.formFiltro = this.fb.group({
      brand: [''],
      color: [''],   // <--- Debe existir este control
      codToday: [''],
      company: [''],
      producto: [''],
      genero: [''],
  ...
  buscar(): void {
    const { brand,color, codToday, company, size: talla } = this.formFiltro.value;
